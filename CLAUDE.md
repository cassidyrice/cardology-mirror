...
- **One product for sale: the $47 One Question Reading** (a written reading on the one question the buyer types before checkout, built from their birth card, this year's Long Range and Pluto cards, and the two karma cards; 500 to 700 words, plain text, emailed within 2 business days; no login, no PDFs, no video). **Fulfilled by hand:** the webhook emails Cass an ACTION line with a paste-ready `reading M/D/YYYY "question" --send buyer@email` command (tool: `~/cardblueprints-ops/card-reading/`, runs on his Claude subscription); nothing is generated on the site. Internal slug/route stays `deep-dive`, SKU `one-question-47`; the live $47 price id lives in the Pages secret `STRIPE_PRICE_BLUEPRINT_BREAKDOWN` (same price object as the retired video, reused on purpose). Public path `/products/one-question-reading`; old product URLs 301 there. Stripe account **Card Blueprint** (`acct_1U1a1dChx1yAVyrs`) only. Never the Cassidy Rice / 52xseven account. Do not resurrect $9/$13/$17/$19/$27/membership CTAs. Fulfillment for past buyers stays intact (the $19 52xSeven year app at `/blueprint?token=…` and the PDFs still resolve). Spec: `~/cardblueprints-ops/plans/one-question-reading-2026-09-13.md`. Voice: `~/cardblueprints-ops/plans/brand-voice-2026-09-13.md`.
...
## Map
- Calculator + $47 CTA: `components/seo/BirthCardCalculator.tsx`, `components/seo/DeepDiveCta.tsx`, `components/checkout/DeepDiveHostedCheckout.tsx` (opens the review page), question field `components/checkout/CheckoutContinueForm.tsx`
