...
- **One product for sale: the $19 52xSeven Blueprint** (a phone-friendly year app at `/blueprint?token=…`, report-token slug `52xseven-blueprint`: birth card with light/shadow read, the current dated 52-day chapter, all seven chapters, and the yearly story arc on one map; unlocks instantly on the confirmation page plus an emailed sign-in link; 12 months of access, one payment, no renewal; no video, no PDFs). Internal slug/route stays `deep-dive`; price id lives in the Pages secret `STRIPE_PRICE_52XSEVEN_BLUEPRINT`. Stripe account **Card Blueprint** (`acct_1U1a1dChx1yAVyrs`) only. Never the Cassidy Rice / 52xseven account. Do not resurrect $13/$27/$17/$47/membership CTAs. Fulfillment for past buyers stays intact.
...
## Map
- Calculator + $19 CTA: `components/seo/BirthCardCalculator.tsx`, `components/seo/DeepDiveCta.tsx`, `components/checkout/DeepDiveHostedCheckout.tsx`
