...
- **One product for sale: the $47 Blueprint Breakdown Video** (5-minute video; bonuses: the $9 Birth Card Deep Dive PDFs, instant, + the Yearly Timing Map). Internal slug/route stays `deep-dive`; price id lives in the Pages secret `STRIPE_PRICE_BLUEPRINT_BREAKDOWN`. The Yearly Timing Map is drawn on request at `/api/timing-map` (report token, `lib/timing-map/`); the video is recorded per buyer and emailed by hand within 2 business days — the intake email says who to record for. Stripe account **Card Blueprint** (`acct_1U1a1dChx1yAVyrs`) only. Never the Cassidy Rice / 52xseven account. Do not resurrect $13/$27/$17/membership CTAs. Fulfillment for past buyers stays intact.
