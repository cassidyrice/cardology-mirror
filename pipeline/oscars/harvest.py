"""Assemble oscars/people.jsonl: Wikipedia infobox DOB + Wikidata P569 verify."""
...
from pipeline.oscars.wiki_list import (
    BEST_ACTOR_URL,
    BEST_ACTRESS_URL,
    OSCARS_DATABASE,
    OSCARS_HOME,
...
SCHEMA_PATH = ROOT / "schema" / "oscar.schema.json"
DEFAULT_OUT = ROOT / "data" / "oscars" / "people.jsonl"
DEFAULT_PROVENANCE = ROOT / "data" / "oscars" / "provenance.json"
...
def validate_oscars(rows: list[dict]) -> None:
    ...
    if errors:
        raise ValueError("oscars people.jsonl failed schema validation:\n" + "\n".join(errors))
...
def unique_slug(name: str, used: set[str], *, title: str) -> str:
    base = slugify(name) or slugify(title) or "oscar-winner"
    if base not in used:
        return base
    extra = slugify(title) or "winner"
    candidate = extra if extra not in used else f"{base}-oscar"
...
def _normalize_awards(awards: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for award in awards:
        ...
        film = str(award.get("film") or "").strip()
        oscars_url = str(award.get("oscars_url") or "").strip()
        if not year or not film or not oscars_url:
            continue
        cleaned.append(
            {
                "year": year,
                "film_year": award.get("film_year"),
                "ceremony_number": int(award["ceremony_number"]),
                "category": str(award["category"]),
                "category_id": str(award["category_id"]),
                "film": film,
...
def classify_row(
    *,
    ...
    draft = {
        ...
        "oscars_url": awards[0]["oscars_url"],
        "awards": awards,
        "wikipedia_infobox_date": wiki_iso,
        "wikidata_birth_date": wikidata_iso,
...
def harvest(
    *,
    ...
    today = today or date.today()
    people, _wikitexts = fetch_winner_rows(cache_dir=cache_dir / "wikipedia" / "oscars_lists")
    infoboxes: dict[str, dict[str, Any]] = {}
    for person in people:
        ...
        birth, _wikitext = fetch_infobox_birth(
            title,
            cache_path=cache_dir / "wikipedia" / "oscars_infobox" / f"{safe}.json",
        )
    ...
    provenance = {
        ...
        "catalog_winners": len(people),
        "catalog_awards": sum(len(person.get("awards") or []) for person in people),
        "kept": len(rows),
        "excluded": len(exclusions),
        "by_reason": by_reason,
        "exclusions": exclusions,
        "sources": [
            f"Academy of Motion Picture Arts and Sciences ceremony pages ({OSCARS_HOME})",
            f"AMPAS Awards Database ({OSCARS_DATABASE})",
            f"Wikipedia Academy Award for Best Actor ({BEST_ACTOR_URL})",
            f"Wikipedia Academy Award for Best Actress ({BEST_ACTRESS_URL})",
        ...
        "rules": {
            "keep": (
                "Wikipedia Best Actor / Best Actress winner tables AND Wikipedia "
                "infobox YYYY-MM-DD AND Wikidata P569 precision=11 AND dates match"
            ),
            "drop": [
            ...
            "supporting_categories": False,
        },
        "oscars_home": OSCARS_HOME,
        "best_actor_url": BEST_ACTOR_URL,
        "best_actress_url": BEST_ACTRESS_URL,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Harvest Academy Award Best Actor/Actress birth-card JSONL (day-precision only)."
    )
    ...
    print(
        f"wrote {prov['kept']} Oscar winners to {args.out} "
