...
DEFAULT_SA_PATH = Path("/tmp/sa.json")
...
def materialize_sa_json(dest: Path = DEFAULT_SA_PATH) -> Path | None:
    """If GOOGLE_APPLICATION_CREDENTIALS is a JSON blob, write it to dest.

    Does not invent credentials. Returns the usable file path or None.
    """
...
def discover_vertex_config() -> VertexConfig:
    ...
    materialized = materialize_sa_json()
    credentials_path = str(materialized) if materialized else (
        (os.environ.get(CREDS_ENV) or "").strip() or None
    )
...
def auth_blocker_message(config: VertexConfig) -> str:
    ...
    lines += [
        ...
        "(or export them in the shell) before a real submit:",
        "",
        ...
        "",
        "Then rerun:",
        "  python3 -m enrich.submit_batch --input enrich/artifacts/vertex_wave_slice3_batch.jsonl",
        "  python3 -m enrich.submit_batch --poll",
        "  python3 -m enrich.parse_results --append --retry-scope enrich/artifacts/wave_slice3_people.jsonl",
    ]
    return "\n".join(lines)
