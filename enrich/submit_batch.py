...
DEFAULT_INPUT = ROOT / "enrich" / "artifacts" / "vertex_batch.jsonl"
...
def _status_payload(
    *,
    ...
    estimate = estimate_batch(input_path, model=config.model) if input_path.is_file() else {}
    payload = {
        "submitted": submitted,
        "reason": reason,
...
def run_submit(args: argparse.Namespace) -> int:
    ...
    write_status(
        ...
        _status_payload(
            ...
            reason="submitted",
            job=job,
            extra={"input_uri": input_uri, "output_prefix": output_prefix},
        ),
    )
