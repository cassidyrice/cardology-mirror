"""Build the remaining-wave Vertex slice from people still missing enrich.

Does not submit a job, invent bios, or start another Vertex job.
"""
...
# Already-decided DROP — do not resubmit.
# Slice-1 TPU CANCELLED empties + slice-2 thin/bad predictions.
PRIOR_DROP_QIDS: frozenset[str] = frozenset(
    {
        "Q3133396",  # Herb Reed — empty prediction (TPU CANCELLED)
        "Q228808",  # Marie-José Pérec — empty prediction (TPU CANCELLED)
        "Q52440",  # LL Cool J — card_in_life 119 words
        "Q501697",  # J. Mike Lounge — prediction JSON is not an object
        "Q348011",  # Joe Montana — card_in_life 186 words
        "Q18778",  # Tímea Nagy — card_in_life 119 words
    }
)

DEFAULT_ENRICHED = ROOT / "pipeline" / "data" / "people_enriched.jsonl"
DEFAULT_PEOPLE_OUT = ROOT / "enrich" / "artifacts" / "wave_slice3_people.jsonl"
DEFAULT_BATCH_OUT = ROOT / "enrich" / "artifacts" / "vertex_wave_slice3_batch.jsonl"
DEFAULT_INVENTORY = ROOT / "enrich" / "artifacts" / "wave_slice3_inventory.json"
DEFAULT_ESTIMATE = ROOT / "enrich" / "artifacts" / "wave_slice3_estimate.json"
DEFAULT_QIDS = ROOT / "enrich" / "artifacts" / "wave_slice3_qids.jsonl"
# Remainder: take every remaining missing wave person after skip.
SLICE_LIMIT = 10_000
THIS_JOB_USD_BAND = 20.0
SLICE_KIND = "wave_slice3"
...
def write_wave_slice(
    *,
    ...
    skip = set(PRIOR_DROP_QIDS if exclude is None else exclude)
    ...
    estimate["notes"] = (
        ...
        f"Remainder {written['requests']} of {stats['unique_missing']} missing wave "
        "people after #104. Do not submit another Vertex job unless asked."
    )
    ...
    inventory = {
        ...
        "dropped_prior": stats["exclude_qids"],
        ...
        "do_not": [
            "submit another Vertex job after this remainder",
            "merge results",
            "deploy",
            "invent bios",
            "resubmit prior DROP empties/thin",
        ],
    }
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Write remaining-wave Vertex JSONL. Does not submit."
    )
    ...
    est = result["estimate"]
    print(f"wrote {result['requests']} wave-slice-3 Vertex requests → {args.out}")
    print(
