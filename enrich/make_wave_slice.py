"""Build the first hub-sized Vertex slice from wave people missing enrich.

Does not submit a job, invent bios, or start slice 2.
"""
...
DEFAULT_PEOPLE_OUT = ROOT / "enrich" / "artifacts" / "wave_slice1_people.jsonl"
DEFAULT_BATCH_OUT = ROOT / "enrich" / "artifacts" / "vertex_wave_slice1_batch.jsonl"
DEFAULT_INVENTORY = ROOT / "enrich" / "artifacts" / "wave_slice1_inventory.json"
DEFAULT_ESTIMATE = ROOT / "enrich" / "artifacts" / "wave_slice1_estimate.json"
DEFAULT_QIDS = ROOT / "enrich" / "artifacts" / "wave_slice1_qids.jsonl"
SLICE_LIMIT = 1079
...
def select_slice(
    *,
    ...
    counted = inventory_missing(pack_paths=pack_paths, enriched=enriched)
    ...
    for pack in hole_order:
        ...
        for row in load_jsonl(path):
            ...
            if not row_is_ready(row):
                continue
            qid = str(row["qid"]).strip()
            if qid in enriched or qid in seen:
                continue
            seen.add(qid)
...
def write_wave_slice(
    *,
    ...
    people, stats = select_slice(pack_paths=packs, enriched=enriched, limit=limit)
    ...
    estimate["label"] = (
        "wave slice 1 — #70 / hub contract (near-verbatim, source_text "
    ...
    estimate["notes"] = (
        ...
        "people. Do not submit slice 2 unless asked."
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Write first hub-sized wave Vertex JSONL. Does not submit."
    )
    ...
    est = result["estimate"]
    print(f"wrote {result['requests']} wave-slice-1 Vertex requests → {args.out}")
    print(
