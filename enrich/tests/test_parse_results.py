...
def test_parse_append_keeps_previous_and_scopes_retry(tmp_path: Path) -> None:
    existing = tmp_path / "people_enriched.jsonl"
    existing.write_text(
        json.dumps({"qid": "Q0FIX1", "slug": "ada-fixture", "enrich_status": "ok"}) + "\n",
        encoding="utf-8",
    )
    scope = tmp_path / "retry_scope.jsonl"
    scope.write_text(
        json.dumps({"qid": "Q0FIX2"}) + "\n" + json.dumps({"qid": "Q0FIX3"}) + "\n",
        encoding="utf-8",
    )
    good = _ok_payload("Bea Fixture")
    predictions = tmp_path / "predictions.jsonl"
    predictions.write_text(json.dumps(_pred("Q0FIX2", good)) + "\n", encoding="utf-8")

    retry = tmp_path / "retry.jsonl"
    report = parse_results(
        people_path=FIXTURE_PEOPLE,
        predictions_path=predictions,
        enriched_path=existing,
        retry_path=retry,
        report_path=tmp_path / "report.json",
        append=True,
        retry_scope_path=scope,
    )

    assert report["previous_kept"] == 1
    assert report["accepted"] == 1
    ...
    assert [row["qid"] for row in rows] == ["Q0FIX1", "Q0FIX2"]
    retry_rows = [json.loads(line) for line in retry.read_text(encoding="utf-8").splitlines()]
    assert {row["qid"] for row in retry_rows} == {"Q0FIX3"}
