    ...
    for i, item in enumerate(evidence):
        ...
        if not ok:
            errors.append(
                ...
                f"(containment {score:.3f} < {threshold:.2f})"
            )
...
def _load_scope_qids(path: Path | None) -> set[str] | None:
    if path is None:
        return None
    if not path.is_file():
        raise SystemExit(f"retry scope not found: {path}")
    qids = {str(row.get("qid") or "").strip() for row in load_jsonl(path)}
    qids.discard("")
    return qids
...
def parse_results(
    *,
    ...
    threshold: float = CONTAINMENT_THRESHOLD,
    append: bool = False,
    retry_scope_path: Path | None = None,
) -> dict[str, Any]:
    ...
    scope_qids = _load_scope_qids(retry_scope_path)

    previous: list[dict[str, Any]] = []
    previous_qids: set[str] = set()
    if append and enriched_path.is_file():
        previous = load_jsonl(enriched_path)
        previous_qids = {str(row.get("qid") or "") for row in previous}
        previous_qids.discard("")
    ...
    expected_people = [
        person
        for person in people
        if str(person.get("qid") or "")
        and (scope_qids is None or str(person.get("qid") or "") in scope_qids)
        and str(person.get("qid") or "") not in previous_qids
    ]
    ...
    outgoing = previous + accepted if append else accepted
    ...
    for row in rejected:
        for reason in row.get("reasons") or ["unknown"]:
            ...
            lowered = key.lower()
            if "containment" in lowered or "near-verbatim" in lowered:
                reason_counts["containment"] += 1
            elif "no prediction" in lowered:
    ...
    report = {
        "people": len(people),
        "expected": len(expected_people),
        "predictions": len(predictions),
        "accepted": len(accepted),
        "accepted_total": len(outgoing),
        "previous_kept": len(previous),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--append",
        action="store_true",
        help="Keep existing people_enriched.jsonl rows and append newly accepted qids.",
    )
    parser.add_argument(
        "--retry-scope",
        type=Path,
        default=None,
        help="Only expect/score qids listed in this retry.jsonl (do not mark others missing).",
    )
    ...
    report = parse_results(
        ...
        append=args.append,
        retry_scope_path=args.retry_scope,
    )
    print(
        f"accepted {report['accepted']} this run "
        f"(total {report['accepted_total']}) → {args.out}; "
