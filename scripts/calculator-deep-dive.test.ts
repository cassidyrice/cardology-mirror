...
test("the product still sits beside the calculator on the pages that get clicks", () => {
  const still = read("components/product/ProductStill.tsx");
  expect(still).toContain("/product/52xseven-");
  expect(still).toContain("DEEP_DIVE_PRODUCT_PATH");
  expect(still).toContain("offer_cta_clicked");
  expect(still).toContain("loading=\"lazy\"");
  // Real screens from the purchased app, regenerated by scripts/product-still.ts.
  expect(existsSync(join(root, "public/product/52xseven-now.png"))).toBe(true);
  expect(existsSync(join(root, "public/product/52xseven-story.png"))).toBe(true);
  expect(read("scripts/product-still.ts")).toContain("PRODUCT_STILL");
  // The harness route must stay 404 in production.
  expect(read("app/internal/product-still/page.tsx")).toContain(
    'process.env.PRODUCT_STILL !== "1"',
  );

  for (const [file, placement] of [
    ["app/birth-card-calculator/page.tsx", "birth-card-calculator-aside"],
    ["app/birth-card-compatibility-calculator/page.tsx", "compatibility-calculator-aside"],
    ["app/cardology-compatibility/page.tsx", "cardology-compatibility-aside"],
    ["app/page.tsx", "home-landing-aside"],
  ] as const) {
    const source = read(file);
    expect(source).toContain("<ProductStill");
    expect(source).toContain(`placement="${placement}"`);
  }
  // The approved bare mobile landing keeps the calculator alone.
  expect(read("app/page.tsx")).toContain('className="hidden lg:block"');
});
