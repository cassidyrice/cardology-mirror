...
test("grounds the existing celebrity blog profiles and leaves CTA link-only", () => {
  ...
  for (const post of cited) {
    ...
    expect(howTo?.links?.some((link) => link.href === "/products/one-question-reading")).toBe(true);
    expect(post.coreLinks.some((link) => link.href === "/products/one-question-reading")).toBe(true);
