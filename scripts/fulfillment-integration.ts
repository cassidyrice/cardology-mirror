...
let writes=0, sends=0; const notifications:any[]=[];
...
globalThis.fetch=(async (url:any,init:any) => {
 ...
 if(href==="https://api.resend.com/emails"){sends++;notifications.push(JSON.parse(init.body));assert.ok(init.headers["Idempotency-Key"]);return Response.json({id:"email_mock"});}
...
console.log("Passed: signed unpaid webhook + retrieval denied; delayed payment fulfills; duplicate event does not regenerate/resend; paid success retrieval; checkout question validation and Stripe metadata. Mock HTTP only.");
