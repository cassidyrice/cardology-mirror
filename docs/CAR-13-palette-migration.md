...
Pass 1 inventoried the layer and proposed the mapping. Cass agreed that mapping, including D1 option (a), D2 (accept the warm-wash shift), and D3 (ink-button preference). Batches A, B, C, D, E, F, G, H, I, and J are the deletions so far.

Status: batch J landed. Rule 3 (batch A), rule 4 (batch B), rules 5–6 (batch C: `.paper-shell [class*="text-gold"]`, `[class*="text-ember"]`, and the anchor override `a[class*="text-gold"]`), rule 7 (batch D: `.paper-shell .text-sage`, `.text-dusk`), rules 8 and 15 (batch E: `.paper-shell .text-ink`, `.paper-shell .bg-foil`), rule 18 (batch F: `.paper-shell [class*="bg-void"]`, `[class*="bg-haze"]`, `[class*="bg-cosmos"]`, `[class*="bg-white"]`), rule 17 (batch G: `.paper-shell [class*="border-white"]`, `[class*="border-bone"]`, `[class*="border-haze"]`, `[class*="border-gold"]`), rule 16 (batch H: `.paper-shell [class*="bg-gold"]`), rules 1 and 2 (batch I: `.paper-shell .starfield::before`, `.paper-shell .bg-cosmic`, `.paper-shell [class*="bg-cosmic"]`), and rule 12 (batch J: `.paper-shell .card-surface`) are deleted. Rules 9–11, 13–14, and 19–23 stay. `tailwind.config.ts` dark palette keys stay. `:root` gained `--club: #183f30` and Tailwind gained `brand.club`. `brand.ivory` is `color-mix` with `<alpha-value>` so `bg-brand-ivory/70` paints; a bare `var(--ivory)` drops that utility. Solid `bg-brand-ivory` stays ivory.
...
Line numbers are `app/globals.css` after batch J deleted rule 12. "Depends" means a descendant of `.paper-shell` whose class attribute matches the selector. Outside-shell files in the appendix use the same class names and do not depend on the rule; they block deleting the Tailwind color, not the rule.
...
| 13 | 628–631 | `.card-frame` | yes | border `rgba(20,17,13,.25)`, fill `rgba(244,240,231,.94)` | K |
| 14 | 633–638 | `.foil-text` | yes | clipped gradient `var(--rust)` → `#c77e63` → `var(--gold)`; shell sets `--rust` to `var(--oxblood)` | K |
...
| 19 | 640–646 | `input`, `textarea`, `select` | yes | cream field, ink text, warm border | M (keep or migrate; see below) |
| 20 | 648–651 | placeholders | yes | `rgba(20,17,13,.45)` | M |
| 21 | 653–659 | `.rounded-2xl` … `.rounded-t-3xl` | yes | radius `0.25rem` | M |
| 22 | 664–667 | `a.rounded-full`, `button.rounded-full` | yes | radius `0.25rem` | M |
| 23 | 669–683 | `.app-paper-stage` and descendants | no | inset shadow, `min-height: auto`, softer `.shadow-lg` | M |
...
| `border-white/*`, `border-bone/*`, `border-haze`, `border-gold/*`, `hover:border-gold`, `focus:border-gold` | `rgba(20,17,13,.16)` | `border-brand-line` (batch G). `card-surface` keeps `!border-brand-line`. After batch J the base edge is the same `var(--line)`. Form controls still take rule 19. The `PlayingCard` frame still takes rule 13. | shifts from warm black to ink-blue line |
...
D1, D3, and the rule 12, 16, 17, and 18 parts of D2 are agreed and applied. D2 for rule 13 is agreed and not applied. D4 and D5 are agreed and not applied. Batch K and later are not this change.
...
| G | rule 17 (deleted) | `border-white`, `border-gold`, `border-haze`, `border-bone`. | Done, D2. Paper borders are `border-brand-line`. `card-surface` edges use `!border-brand-line`. After batch J the base edge is the same `var(--line)`. Form controls still take rule 19. `PlayingCard` paper-shell mounts pass `border-brand-line`; rule 13 still paints the frame until batch K. Left onboarding, `HomepageCalculatorHero`, and unmounted `BottomNav`, `CompareBand`, and `YearPreview`. |
