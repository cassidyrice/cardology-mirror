...
| 9 | 606–610 | `.eyebrow` | no | `var(--ink-soft)`, weight 700 | L |
| 10 | 612–624 | `.prose-reading` and `p` / `li` / `em` / `strong` | no | `color: var(--ink)` (base rule paints `em` gold `#d9b26a`; the override flattens that to ink) | L |
| 11 | 626–628 | `.hairline` | no | `border-color: var(--line)` | L |
...
| 19 | 630–636 | `input`, `textarea`, `select` | yes | cream field, ink text, warm border | M (keep or migrate; see below) |
| 20 | 638–641 | placeholders | yes | `rgba(20,17,13,.45)` | M |
| 21 | 643–649 | `.rounded-2xl` … `.rounded-t-3xl` | yes | radius `0.25rem` | M |
| 22 | 654–657 | `a.rounded-full`, `button.rounded-full` | yes | radius `0.25rem` | M |
| 23 | 659–673 | `.app-paper-stage` and descendants | no | inset shadow, `min-height: auto`, softer `.shadow-lg` | M |
...
5. **D5 — shared primitives.** Agreed. `PlayingCard` paper surface is applied in batch K: `surface="paper"` sets the ivory frame, and the dark frame stays for onboarding. Paper callers of `Eyebrow` still switch to `Kicker` or `type-eyebrow` in batch L; `ProfileForm` keeps `Eyebrow`. `SectionTitle`, `Divider`, and `PositionStack` stay for batch L. No blind replace across `ui.tsx`.

D1, D2, and D3 are agreed and applied. D5 is applied for `PlayingCard`. The `Eyebrow` half of D5 is batch L. D4 is agreed and not applied: rules 19–23 stay. Batch L is not this change.
