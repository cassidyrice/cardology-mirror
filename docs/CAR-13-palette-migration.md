...
Pass 1 inventoried the layer and proposed the mapping. Cass agreed that mapping, including D1 option (a), D2 (accept the warm-wash shift), and D3 (ink-button preference). Batches A, B, C, D, E, F, G, H, and I are the deletions so far.

Status: batch I landed. Rule 3 (batch A), rule 4 (batch B), rules 5–6 (batch C: `.paper-shell [class*="text-gold"]`, `[class*="text-ember"]`, and the anchor override `a[class*="text-gold"]`), rule 7 (batch D: `.paper-shell .text-sage`, `.text-dusk`), rules 8 and 15 (batch E: `.paper-shell .text-ink`, `.paper-shell .bg-foil`), rule 18 (batch F: `.paper-shell [class*="bg-void"]`, `[class*="bg-haze"]`, `[class*="bg-cosmos"]`, `[class*="bg-white"]`), rule 17 (batch G: `.paper-shell [class*="border-white"]`, `[class*="border-bone"]`, `[class*="border-haze"]`, `[class*="border-gold"]`), rule 16 (batch H: `.paper-shell [class*="bg-gold"]`), and rules 1 and 2 (batch I: `.paper-shell .starfield::before`, `.paper-shell .bg-cosmic`, `.paper-shell [class*="bg-cosmic"]`) are deleted. Rules 9–14 and 19–23 stay. `tailwind.config.ts` dark palette keys stay. `:root` gained `--club: #183f30` and Tailwind gained `brand.club`. `brand.ivory` is `color-mix` with `<alpha-value>` so `bg-brand-ivory/70` paints; a bare `var(--ivory)` drops that utility. Solid `bg-brand-ivory` stays ivory.
...
Line numbers are `app/globals.css` after batch I deleted rules 1 and 2. "Depends" means a descendant of `.paper-shell` whose class attribute matches the selector. Outside-shell files in the appendix use the same class names and do not depend on the rule; they block deleting the Tailwind color, not the rule.
...
| 9 | 588–592 | `.eyebrow` | no | `var(--ink-soft)`, weight 700 | L |
| 10 | 594–606 | `.prose-reading` and `p` / `li` / `em` / `strong` | no | `color: var(--ink)` (base rule paints `em` gold `#d9b26a`; the override flattens that to ink) | L |
| 11 | 608–610 | `.hairline` | no | `border-color: var(--line)` | L |
| 12 | 612–622 | `.card-surface` | no | cream grid card; replaces the dark gradient in the base rule | J |
| 13 | 624–627 | `.card-frame` | yes | border `rgba(20,17,13,.25)`, fill `rgba(244,240,231,.94)` | K |
| 14 | 629–634 | `.foil-text` | yes | clipped gradient `var(--rust)` → `#c77e63` → `var(--gold)`; shell sets `--rust` to `var(--oxblood)` | K |
...
| 19 | 636–642 | `input`, `textarea`, `select` | yes | cream field, ink text, warm border | M (keep or migrate; see below) |
| 20 | 644–647 | placeholders | yes | `rgba(20,17,13,.45)` | M |
| 21 | 649–655 | `.rounded-2xl` … `.rounded-t-3xl` | yes | radius `0.25rem` | M |
| 22 | 660–663 | `a.rounded-full`, `button.rounded-full` | yes | radius `0.25rem` | M |
| 23 | 665–679 | `.app-paper-stage` and descendants | no | inset shadow, `min-height: auto`, softer `.shadow-lg` | M |
...
D1, D3, and the rule 16, 17, and 18 parts of D2 are agreed and applied. D2 for rules 12 and 13 is agreed and not applied. D4 and D5 are agreed and not applied. Batch J and later are not this change.
