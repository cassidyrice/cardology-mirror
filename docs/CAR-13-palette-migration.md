...
Pass 1 inventoried the layer and proposed the mapping. Cass agreed that mapping, including D1 option (a), D2 (accept the warm-wash shift), D3 (ink-button preference), D4 (keep the geometry and form chrome), and D5 for shared primitives. Batches A through L are the deletions. Batch M is the docs closeout.

Status: batch M landed. Rule 3 (batch A), rule 4 (batch B), rules 5–6 (batch C: `.paper-shell [class*="text-gold"]`, `[class*="text-ember"]`, and the anchor override `a[class*="text-gold"]`), rule 7 (batch D: `.paper-shell .text-sage`, `.text-dusk`), rules 8 and 15 (batch E: `.paper-shell .text-ink`, `.paper-shell .bg-foil`), rule 18 (batch F: `.paper-shell [class*="bg-void"]`, `[class*="bg-haze"]`, `[class*="bg-cosmos"]`, `[class*="bg-white"]`), rule 17 (batch G: `.paper-shell [class*="border-white"]`, `[class*="border-bone"]`, `[class*="border-haze"]`, `[class*="border-gold"]`), rule 16 (batch H: `.paper-shell [class*="bg-gold"]`), rules 1 and 2 (batch I: `.paper-shell .starfield::before`, `.paper-shell .bg-cosmic`, `.paper-shell [class*="bg-cosmic"]`), rule 12 (batch J: `.paper-shell .card-surface`), rules 13 and 14 (batch K: `.paper-shell .card-frame`, `.paper-shell .foil-text`), and rules 9–11 (batch L: `.paper-shell .eyebrow`, `.paper-shell .prose-reading`, `.paper-shell .hairline`) are deleted. All 18 palette remaps are done (0 remaining). CAR-13 acceptance for the palette layer is met. D4 applied: rules 19–23 stay as intentional paper-shell chrome (inputs and textareas and selects, placeholders, the rounded-radius overrides, `a.rounded-full` and `button.rounded-full`, `.app-paper-stage`). They were not deleted and were not migrated onto utilities. They are closed out of CAR-13. Chrome stays by design. `tailwind.config.ts` dark palette keys stay. Removing those keys is a separate final PR, when the last consumers are gone (`body`'s `bg-ink text-bone`, onboarding, access, the dark `PlayingCard` frame, and the rest). Not this PR. `:root` gained `--club: #183f30` and Tailwind gained `brand.club`. `brand.ivory` is `color-mix` with `<alpha-value>` so `bg-brand-ivory/70` paints; a bare `var(--ivory)` drops that utility. Solid `bg-brand-ivory` stays ivory.

`docs/SITE-RECORD.md` §3 records the unlock Worker webhook decision. This file remains the §1.1 palette tracking note; the rest of the 2026-09-21 audit is not reconstructed there. Palette layer: 0 of 18 remaps remaining. Chrome rules 19–23 stay (D4).

## Tracking

The inventory was counted on `main` at `f040184` (the paper-shell remap block was lines 585–747). A deletion of the blueprint-ambient rules earlier in that file had moved this block up from the 739–901 range named in the issue. Batch A removed rule 3. Batch B removed rule 4. Batch C removed rules 5 and 6. Batch D removed rule 7 and added `--club` on `:root`. Batch E removed rules 8 and 15. Batch F removed rule 18. Batch G removed rule 17. Batch H removed rule 16. Batch I removed rules 1 and 2. Batch J removed rule 12 and moved the paper card onto the base `.card-surface` (lines 135–145), with `.card-surface-dark` at lines 147–152. Batch K removed rules 13 and 14. Batch L removed rules 9, 10, and 11. The base `.prose-reading` rule is lines 122–137. The base `.eyebrow` rule is lines 103–109. The base `.hairline` rule is line 140. The base `.foil-text` rule is lines 167–173. The block now ends at line 660. Batch M does not move it. Appendix counts below are the pre-batch-A inventory.

| Slice | Rule groups | Remaining |
| --- | ---: | --- |
| Palette and surface remaps (the layer CAR-13 exists to remove) | 18 | 0 (0%). Done. |
| Geometry and form chrome (radius, inputs, `.app-paper-stage`) | 5 | kept. Closed out of CAR-13 (D4). |
| Whole `.paper-shell` compatibility block | 23 | 0 palette rules left. 5 chrome rules remain in CSS by design. |
...
Line numbers are `app/globals.css` after batch L deleted rules 9–11. Batch M does not move them. "Depends" means a descendant of `.paper-shell` whose class attribute matches the selector. Outside-shell files in the appendix use the same class names and do not depend on the rule; they block deleting the Tailwind color, not the rule.
...
| 19 | 617–623 | `input`, `textarea`, `select` | yes | cream field, ink text, warm border | kept (D4, batch M) |
| 20 | 625–628 | placeholders | yes | `rgba(20,17,13,.45)` | kept (D4, batch M) |
| 21 | 630–636 | `.rounded-2xl` … `.rounded-t-3xl` | yes | radius `0.25rem` | kept (D4, batch M) |
| 22 | 641–644 | `a.rounded-full`, `button.rounded-full` | yes | radius `0.25rem` | kept (D4, batch M) |
| 23 | 646–660 | `.app-paper-stage` and descendants | no | inset shadow, `min-height: auto`, softer `.shadow-lg` | kept (D4, batch M) |

Rules 19–23 are chrome. They are not dark-palette class names. D4 applied in batch M: Cass kept them as the paper shell's intentional geometry. They stay in `app/globals.css`. They were not migrated onto utilities. They are closed out of CAR-13. The 18 palette remaps are done.
...
| `border-white/*`, `border-bone/*`, `border-haze`, `border-gold/*`, `hover:border-gold`, `focus:border-gold` | `rgba(20,17,13,.16)` | `border-brand-line` (batch G). `card-surface` keeps `!border-brand-line`. After batch J the base edge is the same `var(--line)`. Form controls still take rule 19, which batch M kept. The paper `PlayingCard` frame is `border-brand-line` (batch K). | shifts from warm black to ink-blue line |
| `bg-void`, `bg-haze`, `bg-cosmos`, `bg-white` and opacities | cream `rgba(244,240,231,.68)` | `bg-brand-ivory/70` (batch F). Form controls still take rule 19's cream field, which batch M kept. | shifts from warm cream to ivory |
...
| `.app-paper-stage` | stage chrome | Kept (D4, batch M). The class and rule 23 stay. Two call sites: `AppFeaturePage`, `app/52-day-period-meaning-tool/page.tsx`. | kept |
...
4. **D4 — geometry rules 19–23.** Agreed: keep them. Applied in batch M. They are the paper shell, not the dark palette. Rules 19–23 stay in `app/globals.css` (inputs, placeholders, radius overrides, `a`/`button.rounded-full`, `.app-paper-stage`). They were not migrated onto utilities. The 18 palette remaps are at 0 remaining, so CAR-13 acceptance for the palette layer is met. The 5 chrome rules are closed out of this issue and stay by design.
...
D1, D2, D3, D4, and D5 are agreed and applied. D4 (batch M) keeps rules 19–23 as paper-shell chrome. No further palette batch remains. Removing dark keys from `tailwind.config.ts` is a separate final PR, not this closeout.

## Batch plan

Each batch is one PR. Batches A–L migrate only the files listed for that rule, screenshot every route those files mount, and delete the rule in the same PR. Exit check for those batches: the selector's class no longer occurs on a paper-shell screen. Outside-shell and unmounted files may still contain the old class; they are called out so the rule is not kept "just in case," and so `tailwind.config.ts` is not edited yet. Batch M is the exception: D4 keeps rules 19–23, so that closeout deletes nothing and changes no class strings.
...
| M | none (D4 keep) | docs closeout only: radius utilities, form controls, `.app-paper-stage` | Done. Rules 19–23 stay as paper-shell chrome. Not deleted. Not migrated onto utilities. Palette remaps: 0 remaining. CAR-13 palette-layer acceptance is met. |

Final PR, separate from the 18 and from batch M: remove dark palette keys from `tailwind.config.ts` when `app/` and `components/` no longer reference them. That includes `body`'s `bg-ink text-bone`, onboarding, access, and any dark `PlayingCard` frame that still wants haze/cosmos. Not this PR. Not any palette batch above. Do not start that cleanup in the batch M closeout.
