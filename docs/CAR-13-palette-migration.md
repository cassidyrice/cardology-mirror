...
Pass 1 inventoried the layer and proposed the mapping. Cass agreed that mapping, including D1 option (a), D2 (accept the warm-wash shift), and D3 (ink-button preference). Batches A, B, C, D, E, F, and G are the deletions so far.

Status: batch G landed. Rule 3 (batch A), rule 4 (batch B), rules 5–6 (batch C: `.paper-shell [class*="text-gold"]`, `[class*="text-ember"]`, and the anchor override `a[class*="text-gold"]`), rule 7 (batch D: `.paper-shell .text-sage`, `.text-dusk`), rules 8 and 15 (batch E: `.paper-shell .text-ink`, `.paper-shell .bg-foil`), rule 18 (batch F: `.paper-shell [class*="bg-void"]`, `[class*="bg-haze"]`, `[class*="bg-cosmos"]`, `[class*="bg-white"]`), and rule 17 (batch G: `.paper-shell [class*="border-white"]`, `[class*="border-bone"]`, `[class*="border-haze"]`, `[class*="border-gold"]`) are deleted. Rules 1–2, 9–14, 16, and 19–23 stay. `tailwind.config.ts` dark palette keys stay. `:root` gained `--club: #183f30` and Tailwind gained `brand.club`. `brand.ivory` is `color-mix` with `<alpha-value>` so `bg-brand-ivory/70` paints; a bare `var(--ivory)` drops that utility. Solid `bg-brand-ivory` stays ivory.
...
`PlayingCard`, `CardFace`, `CardBack`, and `FoilSheen` mount inside paper-shell app cards and inside onboarding (`ProfileForm`). `surface="paper"` only recolors pips. Paper-shell mounts pass `frameBorder="border-brand-line"`. The default frame border stays `border-white/10` for `ProfileForm` and `HomepageCalculatorHero`. The rest of the dark frame stays for batch K: `card-frame`, `from-haze`, `to-cosmos`, `eyebrow`, `ring-gold`, `foil-text`, `bg-foil`. Rule 13 still paints the paper frame border warm. The live homepage does not mount `PlayingCard` (tests import `HomepageCalculatorHero`; `app/page.tsx` renders `LandingCalculator`).
...
Line numbers are `app/globals.css` after batch G deleted rule 17. "Depends" means a descendant of `.paper-shell` whose class attribute matches the selector. Outside-shell files in the appendix use the same class names and do not depend on the rule; they block deleting the Tailwind color, not the rule.

Selector behavior that is easy to miss:

- Rule 8's exact `.text-ink` selector, rule 7's exact `.text-sage` and `.text-dusk` selectors, rule 17's `[class*="border-white"]`, `[class*="border-bone"]`, `[class*="border-haze"]`, and `[class*="border-gold"]` selectors, and rule 18's `[class*="bg-void"]`, `[class*="bg-haze"]`, `[class*="bg-cosmos"]`, and `[class*="bg-white"]` selectors are deleted. `hover:text-bone` does not match `.text-bone`.
- `text-mist`, `text-faint`, `text-gold`, `text-ember`, and `bg-gold` use `[class*="…"]`, so variants and opacities match.
...
- `from-haze`, `to-cosmos`, `via-cosmos`, `ring-gold`, `bg-ink`, `bg-ember`, `bg-bone`, `text-void`, `text-white` match no rule. Inside the shell they paint the dark palette on paper. `bg-sage` and `bg-dusk` left the paper screens in batch D. `bg-void`, `bg-haze`, and `bg-white` left the paper screens in batch F. `border-white`, `border-gold`, and `border-haze` left the paper screens in batch G. `border-bone` was never on a paper screen.

| # | Lines | Selector (abbreviated) | `!important` | What the visitor sees inside the shell | Delete with batch |
| --- | --- | --- | --- | --- | --- |
| 1 | 588–591 | `.starfield::before` | no | starfield removed | I |
| 2 | 593–596 | `.bg-cosmic`, `[class*="bg-cosmic"]` | yes | background transparent | I |
...
| 9 | 598–602 | `.eyebrow` | no | `var(--ink-soft)`, weight 700 | L |
| 10 | 604–616 | `.prose-reading` and `p` / `li` / `em` / `strong` | no | `color: var(--ink)` (base rule paints `em` gold `#d9b26a`; the override flattens that to ink) | L |
| 11 | 618–620 | `.hairline` | no | `border-color: var(--line)` | L |
| 12 | 622–632 | `.card-surface` | no | cream grid card; replaces the dark gradient in the base rule | J |
| 13 | 634–637 | `.card-frame` | yes | border `rgba(20,17,13,.25)`, fill `rgba(244,240,231,.94)` | K |
| 14 | 639–644 | `.foil-text` | yes | clipped gradient `var(--rust)` → `#c77e63` → `var(--gold)`; shell sets `--rust` to `var(--oxblood)` | K |
| 15 | — | `.bg-foil` | yes | deleted in batch E; paper buttons use `ink-button`. Decorative foil on cards is batch K | done |
| 16 | 646–648 | `[class*="bg-gold"]` | yes | `background-color: rgba(158,61,36,.12)` (old warm red at 12%) | H |
...
| 19 | 650–656 | `input`, `textarea`, `select` | yes | cream field, ink text, warm border | M (keep or migrate; see below) |
| 20 | 658–661 | placeholders | yes | `rgba(20,17,13,.45)` | M |
| 21 | 663–669 | `.rounded-2xl` … `.rounded-t-3xl` | yes | radius `0.25rem` | M |
| 22 | 674–677 | `a.rounded-full`, `button.rounded-full` | yes | radius `0.25rem` | M |
| 23 | 679–693 | `.app-paper-stage` and descendants | no | inset shadow, `min-height: auto`, softer `.shadow-lg` | M |
...
| `bg-foil` on `FoilSheen` / `CardBack` | rule 15 forced `var(--ink)`; the utility is the gold foil gradient | left for batch K. After rule 15, paper cards show that gradient. | shifts on paper cards |
...
| `border-white/*`, `border-bone/*`, `border-haze`, `border-gold/*`, `hover:border-gold`, `focus:border-gold` | `rgba(20,17,13,.16)` | `border-brand-line` (batch G). `card-surface` uses `!border-brand-line`. Form controls still take rule 19. The `PlayingCard` frame still takes rule 13. | shifts from warm black to ink-blue line |
...
`bg-cosmos` had zero consumers. Batch F deleted that selector with rule 18. `to-cosmos` does not match `[class*="bg-cosmos"]` and stays for batch K.
...
2. **D2 — warm washes vs cyanotype.** Agreed: accept the shift on rules 12, 13, 16, 17, and 18 (cream, warm border, old red `bg-gold`) onto `bg-brand-ivory`, `border-brand-line`, and `bg-brand-gold-soft`. Rule 18 is applied in batch F (`bg-brand-ivory/70`). Rule 17 is applied in batch G (`border-brand-line`). Rules 12, 13, and 16 are still the warm leftovers until batches J, K, and H.
...
D1, D3, and the rule 17 and rule 18 parts of D2 are agreed and applied. D2 for rules 12, 13, and 16 is agreed and not applied. D4 and D5 are agreed and not applied. Batch H and later are not this change.
...
| G | rule 17 (deleted) | `border-white`, `border-gold`, `border-haze`, `border-bone`. | Done, D2. Paper borders are `border-brand-line`. `card-surface` edges use `!border-brand-line` so rule 12 does not keep the warm edge. Form controls still take rule 19. `PlayingCard` paper-shell mounts pass `border-brand-line`; rule 13 still paints the frame until batch K. Left onboarding, `HomepageCalculatorHero`, and unmounted `BottomNav`, `CompareBand`, and `YearPreview`. |
| H | rule 16 | `bg-gold` washes that are not the D3 buttons. | D2 agreed. Do after E so the button pairs are already gone. E has landed. |
...
| J | rule 12 | `.card-surface` | 31 files, all paper or unmounted (`Brand`). Move the cream card onto the base class, or replace with `border border-brand-line bg-brand-ivory`. D2 is agreed. |
