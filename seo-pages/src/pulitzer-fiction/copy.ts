...
function evidenceFromSummary(person: PulitzerRow, sentences: string[]): string[] {
  ...
  if (evidence.length < 3) {
    evidence.push(
      `No extra biographical facts were written for ${person.name} beyond that summary and the Pulitzer Fiction record.`,
    );
  }
  return evidence;
}
