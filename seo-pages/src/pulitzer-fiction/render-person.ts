...
export function renderPulitzerPage(
  person: PulitzerRow,
  meaning: CardMeaning,
  sameCard: readonly PulitzerRow[],
): string {
  const path = pulitzerPath(person.slug);
  const copy = pulitzerCopy(person, meaning);
  ...
  const crumbs = [
    { name: "Home", href: "/" },
    { name: "Pulitzer Fiction", href: "/pulitzer/fiction" },
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: pulitzerOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ...
      : listed
          .map(
            (other) =>
              `<li><a href="${escapeHtml(pulitzerPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(awardPhrase(other.awards))}</li>`,
          )
          .join("\n        ") +
        (extra > 0
  ...
  const awardList = person.awards
    .map((award) => {
      const share = award.shared ? " · joint recipient" : "";
      return `<li><a href="${escapeHtml(award.pulitzer_url)}">${escapeHtml(award.year)} Pulitzer Prize for Fiction</a> — ${escapeHtml(award.work)}${escapeHtml(share)}</li>`;
    })
    .join("\n        ");
  ...
  const body = `
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a Pulitzer Prize for Fiction.
    </p>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not a Pulitzer reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(pulitzerCheckoutHref(person.slug))}">
    ...
    <p class="sources" data-slot="sources">
      Sources:
      Pulitzer.org
      (<a href="${escapeHtml(person.pulitzer_url)}">year record</a>,
      <a href="https://www.pulitzer.org/">pulitzer.org</a>)
      + Wikipedia CC BY-SA 4.0
      (<a href="${escapeHtml(FICTION_WIKI)}">Fiction winners list</a>,
      <a href="${escapeHtml(person.source_url)}">${escapeHtml(person.wikipedia_title)}</a>)
