...
export function renderTimePotyPage(
  person: TimePotyRow,
  meaning: CardMeaning,
  sameCard: readonly TimePotyRow[],
): string {
  const path = timePotyPath(person.slug);
  const copy = timePotyCopy(person, meaning);
  ...
  const crumbs = [
    { name: "Home", href: "/" },
    { name: "TIME Person of the Year", href: "/time-person-of-the-year" },
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: timePotyOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ...
      : listed
          .map(
            (other) =>
              `<li><a href="${escapeHtml(timePotyPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(honorPhrase(other.honors))}</li>`,
          )
          .join("\n        ") +
        (extra > 0
  ...
  const body = `
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a TIME Person of the Year.
    </p>
    ...
    <section data-slot="awards">
      <h2>TIME Person of the Year record</h2>
      <ul>
        ${honorList}
      </ul>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not a TIME reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(timePotyCheckoutHref(person.slug))}">
