...
import { scotusCopy, sourceProse } from "./copy";
import type { CardMeaning, ScotusRow } from "./types";
import { cardMeaningPath, officePhrase, scotusCheckoutHref, scotusOgSlot, scotusPath } from "./urls";
...
export function renderScotusPage(
  person: ScotusRow,
  meaning: CardMeaning,
  sameCard: readonly ScotusRow[],
): string {
  const path = scotusPath(person.slug);
  const copy = scotusCopy(person, meaning);
  ...
  const crumbs = [
    { name: "Home", href: "/" },
    { name: "SCOTUS justices", href: "/scotus" },
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: scotusOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ? `<li data-placeholder="true">No other sitting justice in this set shares the ${escapeHtml(meaning.label)}.</li>`
      : sameCard
          .map(
            (other) =>
              `<li><a href="${escapeHtml(scotusPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(officePhrase(other.role))}</li>`,
          )
          .join("\n        ");
  ...
  const body = `
    <header class="hero">
      <p class="eyebrow">Sitting justice · birth-card coordinate</p>
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a Supreme Court seat.
    </p>
    ...
    <section data-slot="same-card">
      <h2>Other sitting justices with this card</h2>
      <ul>
        ${sameCardList}
      </ul>
    </section>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not a Supreme Court reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(scotusCheckoutHref(person.slug))}">
    ...
    <p class="sources" data-slot="sources">
      Sources:
      SCOTUS.gov Current Members
      (<a href="${escapeHtml(person.scotus_url)}">biographies.aspx</a>)
      + Wikidata CC0 (<a href="https://www.wikidata.org/wiki/${escapeHtml(person.qid)}">${escapeHtml(person.qid)}</a>)
      + Wikipedia CC BY-SA 4.0
      (<a href="${escapeHtml(person.source_url)}">${escapeHtml(person.wikipedia_title)}</a>).
      Birth date ${escapeHtml(person.birth_date)} is the SCOTUS.gov day-precision date, verified against Wikidata P569
