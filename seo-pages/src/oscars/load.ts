...
import type { CardMeaning, HeldOscar, OscarAward, OscarRow } from "./types";
import { reservedOscarSlugReason } from "./urls";

export type OscarsProvenance = {
  people_count: number;
  catalog_winners: number;
  kept: number;
  excluded: number;
  exclusions: HeldOscar[];
};

export function loadOscarRows(filePath: string): OscarRow[] {
  const people: OscarRow[] = [];
  ...
  for (const person of people) {
    if (slugs.has(person.slug)) {
      throw new Error(`Duplicate Oscar slug: ${person.slug}`);
    }
...
export function loadOscarsProvenance(filePath: string): OscarsProvenance {
...
function normalizeRow(raw: unknown, line: number): OscarRow {
  if (!isRecord(raw)) {
    throw new Error(`Line ${line}: oscar row must be an object`);
  }
  const slug = requiredString(raw, "slug", line);
  const reserved = reservedOscarSlugReason(slug) ?? reservedPersonSlugReason(slug);
  if (reserved) {
  ...
  return {
    ...
    oscars_url: requiredString(raw, "oscars_url", line),
...
function requiredAwards(raw: Record<string, unknown>, line: number): OscarAward[] {
  ...
  return value.map((item, index) => {
    ...
    if (categoryId !== "best-actor" && categoryId !== "best-actress") {
      throw new Error(`Line ${line}: awards[${index}].category_id must be best-actor or best-actress`);
    }
    ...
    return {
      ...
      film_year: optionalNullableString(item, "film_year", line),
      ceremony_number: ceremony,
      category: requiredString(item, "category", line),
      category_id: categoryId,
      film: requiredString(item, "film", line),
      oscars_url: requiredString(item, "oscars_url", line),
    };
  });
}

function normalizeHeld(raw: unknown, index: number): HeldOscar {
  if (!isRecord(raw)) {
