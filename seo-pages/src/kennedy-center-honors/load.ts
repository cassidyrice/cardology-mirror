...
import type {
  CardMeaning,
  HeldKennedyCenter,
  KennedyCenterCrosscheck,
  KennedyCenterHonor,
  KennedyCenterRow,
} from "./types";
import { reservedKennedyCenterSlugReason } from "./urls";

export type KennedyCenterProvenance = {
  people_count: number;
  catalog_acts: number;
  ...
  kept_member: number;
  excluded: number;
  exclusions: HeldKennedyCenter[];
};

const ORG_CROSSCHECKS = new Set<KennedyCenterCrosscheck>(["match", "not_published", "unavailable"]);

export function loadKennedyCenterRows(filePath: string): KennedyCenterRow[] {
  const people: KennedyCenterRow[] = [];
  ...
  for (const person of people) {
    if (slugs.has(person.slug)) {
      throw new Error(`Duplicate Kennedy Center Honors slug: ${person.slug}`);
    }
...
export function loadKennedyCenterProvenance(filePath: string): KennedyCenterProvenance {
...
function normalizeRow(raw: unknown, line: number): KennedyCenterRow {
  if (!isRecord(raw)) {
    throw new Error(`Line ${line}: kennedy-center-honors row must be an object`);
  }
  const slug = requiredString(raw, "slug", line);
  const reserved = reservedKennedyCenterSlugReason(slug) ?? reservedPersonSlugReason(slug);
  if (reserved) {
  ...
  const orgCheck = requiredString(raw, "kennedy_center_crosscheck", line);
  if (!ORG_CROSSCHECKS.has(orgCheck as KennedyCenterCrosscheck)) {
    throw new Error(`Line ${line}: kennedy_center_crosscheck must be match, not_published, or unavailable`);
  }
  ...
  return {
    ...
    kennedy_center_url: requiredString(raw, "kennedy_center_url", line),
    ...
    kennedy_center_birth_date: optionalNullableString(raw, "kennedy_center_birth_date", line),
    kennedy_center_crosscheck: orgCheck as KennedyCenterCrosscheck,
  };
}

function requiredHonors(raw: Record<string, unknown>, line: number): KennedyCenterHonor[] {
  ...
  return value.map((item, index) => {
    ...
    if (role !== "solo" && role !== "member") {
      throw new Error(`Line ${line}: honors[${index}].role must be solo or member`);
    }
    return {
      ...
      role,
      kennedy_center_url: requiredString(item, "kennedy_center_url", line),
    };
  });
}

function normalizeHeld(raw: unknown, index: number): HeldKennedyCenter {
  if (!isRecord(raw)) {
