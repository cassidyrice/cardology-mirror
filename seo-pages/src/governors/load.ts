...
import { reservedGovernorSlugReason } from "./urls";

export type GovernorsProvenance = {
  people_count: number;
  catalog_sitting: number;
  kept: number;
  excluded: number;
  exclusions: HeldGovernor[];
};

export function loadGovernorRows(filePath: string): GovernorRow[] {
  const people: GovernorRow[] = [];
  ...
  for (const person of people) {
    if (slugs.has(person.slug)) {
      throw new Error(`Duplicate governor slug: ${person.slug}`);
    }
...
export function loadGovernorsProvenance(filePath: string): GovernorsProvenance {
...
function normalizeRow(raw: unknown, line: number): GovernorRow {
  if (!isRecord(raw)) {
    throw new Error(`Line ${line}: governor must be an object`);
  }
  const slug = requiredString(raw, "slug", line);
  const reserved = reservedGovernorSlugReason(slug) ?? reservedPersonSlugReason(slug);
  if (reserved) {
  ...
  return {
    ...
    nga_url: optionalNullableString(raw, "nga_url", line),
    ballotpedia_url: requiredString(raw, "ballotpedia_url", line),
  };
}

function normalizeHeld(raw: unknown, index: number): HeldGovernor {
  if (!isRecord(raw)) {
