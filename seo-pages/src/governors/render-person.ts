...
import { governorCopy, sourceProse } from "./copy";
...
export function renderGovernorPage(
  person: GovernorRow,
  meaning: CardMeaning,
  sameCard: readonly GovernorRow[],
): string {
  const path = governorPath(person.slug);
  const copy = governorCopy(person, meaning);
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: governorOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ? `<li data-placeholder="true">No other sitting governor in this set shares the ${escapeHtml(meaning.label)}.</li>`
      : sameCard
          .map(
            (other) =>
              `<li><a href="${escapeHtml(governorPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(other.state)}</li>`,
          )
          .join("\n        ");

  const showJoker = person.card === "Joker" || isJokerDate(person.birth_date);
  const ngaLine = person.nga_url
    ? ` + NGA (<a href="${escapeHtml(person.nga_url)}">${escapeHtml(person.state)}</a>)`
    : " + NGA roster";
  ...
  const body = `
    <header class="hero">
      <p class="eyebrow">Governor · birth-card coordinate</p>
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a governorship.
    </p>
    ...
    <section data-slot="same-card">
      <h2>Other sitting governors with this card</h2>
      <ul>
        ${sameCardList}
      </ul>
    </section>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not a gubernatorial reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(governorCheckoutHref(person.slug))}">
    ...
    <p class="sources" data-slot="sources">
      ...
      (<a href="${escapeHtml(person.source_url)}">${escapeHtml(person.wikipedia_title)}</a>;
      <a href="https://en.wikipedia.org/wiki/List_of_current_United_States_governors">current governors list</a>,
      ${escapeHtml(person.dob_crosscheck)})
      + Wikidata CC0
      (<a href="${escapeHtml(wikidataPath(person.qid))}">${escapeHtml(person.qid)}</a>)
      ${ngaLine}
      + <a href="${escapeHtml(person.ballotpedia_url)}">Ballotpedia</a>.
    </p>
