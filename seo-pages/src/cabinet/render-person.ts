...
import { cabinetCopy, sourceProse } from "./copy";
...
export function renderCabinetPage(
  person: CabinetRow,
  meaning: CardMeaning,
  sameCard: readonly CabinetRow[],
): string {
  const path = cabinetPath(person.slug);
  const copy = cabinetCopy(person, meaning);
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: cabinetOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ? `<li data-placeholder="true">No other sitting Cabinet officer in this set shares the ${escapeHtml(meaning.label)}.</li>`
      : sameCard
          .map(
            (other) =>
              `<li><a href="${escapeHtml(cabinetPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(officePhrase(other.office, other.acting))}</li>`,
          )
          .join("\n        ");
  ...
  const body = `
    <header class="hero">
      <p class="eyebrow">Cabinet · birth-card coordinate</p>
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a Cabinet post.
    </p>
    ...
    <section data-slot="same-card">
      <h2>Other sitting Cabinet officers with this card</h2>
      <ul>
        ${sameCardList}
      </ul>
    </section>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not a Cabinet reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(cabinetCheckoutHref(person.slug))}">
    ...
    <p class="sources" data-slot="sources">
      Sources:
      <a href="${escapeHtml(person.whitehouse_url)}">White House cabinet</a>
