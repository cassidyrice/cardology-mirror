...
export function astronautCopy(person: AstronautRow, meaning: CardMeaning): AstronautCopy {
  ...
  const faqs: AstronautFaq[] = [
    ...
    {
      question: `Does a birth card predict a spaceflight?`,
      answer: `No. These pages are coordinates, not fortune-telling. A birth card names a calendar day in a 52-card year. It does not forecast missions, character, or fate.`,
    },
    {
      question: `Where do ${person.name}'s dates come from?`,
      answer: dateSourceAnswer(person, corps),
    },
  ];
...
function dateSourceAnswer(person: AstronautRow, corps: string): string {
  return (
    `Birth date ${person.birth_date} is the day-precision date on ${person.name}'s NASA astronaut biography (${corps}), ` +
    `verified against Wikidata P569 (CC0, day precision, Gregorian preferred). ` +
    `The two sources match. Year-only NASA dates and NASA↔Wikidata conflicts are dropped, not guessed. ` +
