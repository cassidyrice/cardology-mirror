...
import { astronautCopy, sourceProse } from "./copy";
import type { AstronautRow, CardMeaning } from "./types";
import {
  astronautCheckoutHref,
  astronautOgSlot,
  astronautPath,
  cardMeaningPath,
  corpsPhrase,
...
export function renderAstronautPage(
  person: AstronautRow,
  meaning: CardMeaning,
  sameCard: readonly AstronautRow[],
): string {
  const path = astronautPath(person.slug);
  const copy = astronautCopy(person, meaning);
  ...
  const description = `${person.name}, ${corps}, was born ${dateLabel}. The birth-card coordinate for ${formatMonthDay(month, day)} is the ${meaning.label}. Coordinates, not fortune-telling.`;
  const crumbs = [
    { name: "Home", href: "/" },
    { name: "NASA astronauts", href: "/astronauts" },
  ...
  const jsonLd = jsonLdGraph([
    personJsonLd({
      ...
      image: astronautOgSlot(person.slug),
    }),
  ...
  const sameCardList =
    sameCard.length === 0
      ? `<li data-placeholder="true">No other astronaut in this set shares the ${escapeHtml(meaning.label)}.</li>`
      : listed
          .map(
            (other) =>
              `<li><a href="${escapeHtml(astronautPath(other.slug))}">${escapeHtml(other.name)}</a> — ${escapeHtml(corpsPhrase(other))}</li>`,
          )
          .join("\n        ") +
        (extra > 0
          ? `\n        <li data-same-card-more="true">${extra} more astronauts share this card.</li>`
          : "");
  ...
  const body = `
    <header class="hero">
      <p class="eyebrow">NASA astronaut · birth-card coordinate</p>
      <h1 data-slot="h1">${escapeHtml(h1)}</h1>
      <p class="meta" data-slot="corps">${escapeHtml(corps)}</p>
    ...
    <p class="coordinate-note" data-slot="coordinate-note" role="note">
      ...
      and does not predict a spaceflight.
    </p>
    ...
    <section data-slot="same-card">
      <h2>Other astronauts with this card</h2>
      <ul>
        ${sameCardList}
      </ul>
    </section>
    ...
    <section data-slot="cta">
      ...
      <p>The Blueprint Breakdown is a 5-minute video about your own birth card, with the written Deep Dive as a bonus — not an astronaut reading.</p>
      <p>
        <a class="cta"
           data-checkout-link="true"
           href="${escapeHtml(astronautCheckoutHref(person.slug))}">
    ...
    <p class="sources" data-slot="sources">
      ...
      (<a href="${escapeHtml(person.nasa_url)}">${escapeHtml(person.name)}</a>)
      ...
      (<a href="${escapeHtml(person.source_url)}">${escapeHtml(person.wikipedia_title)}</a>).
      Birth date ${escapeHtml(person.birth_date)} is the NASA day-precision date, verified against Wikidata P569
